...
export class LoginRideComponent extends Pagination implements OnInit {
  ...
  loginGroup = new MatTableDataSource<any>([]);
  ...
  getAllLoginRoster() {
    ...
    this.selectedRow = [];
    let data = { ...this.changeDateFormat(this.filterForm.value), ...this.validPageOptions, shiftType: 'login' };
    this._RoasterListService.getGroupsForAssignCab(data)
      .subscribe(
        response => {
          this.loginGroup.data = response.data.userList;
          // Added index to map - Shivakumar A
          this.loginGroup.data.map((x, index)=>{
            if ( x.cab ){
  ...
  getCabs(id, type, time, tripTime, value) {
    console.log(this.slectedVendorAndCab);
    let data = {
      vendorId: id,
      shiftTime: time,
      shiftType: type,
          ...
          this.slectedVendorAndCab.map(x => { // filter the result on the basis of vendor id
            ...
            if (this.selectedVendorId === x.vendorId) {
              if(time===x.shiftTime){
                ...
                currentRoster=moment(time, 'HH:mm').subtract(tripTime,'minutes');
                ...
                rosterObject=moment(x.shiftTime,'HH:mm').subtract(x.tripTime,'minutes');
  ...
  */
  ...
  opencab(groupId: string, flag: boolean): void {
    ...
    if (flag) {
      ...
      dialogConfig.width = '500px';
      // new roster flow -- satyam
      temp['validFrom']=this.filterForm.value.fromDate;
      temp['validTill']=this.filterForm.value.toDate;
